...
public class UserController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//form에서 전송할 때 사용할 URL과 Servlet이 처리하는 URL이같은지 확인
		System.out.println(command);	
		
		
		//계속사용되기 때문에 밖으로 빼서 선언해줬다. 
		...
		case "user/login":
			...
			//../가 메인인 이유는 현재요청이 user/login이므로 현재위치는 user -> 메인으로 가려면 user의 상위로 이동해야하기 때문이다.
		...
		case "user/insert": 
			...
			if(r==true) {
				...
				//회원가입에 실패 했을 때 처리 
				session.setAttribute("registermsg", "회원가입에 실패하셨습니다.");
		...
		case "user/emailcheck": 
			...
			//json으로 출력할때는 리턴받은 데이터를 바로 저장하지 말고 JSON객체로 변환해서 저장 
